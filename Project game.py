...
bg_game = pygame.image.load("Graphic/grass2.png")
...
def congratulation_screen():
    # โหลดภาพพื้นหลังและข้อความแสดงความยินดี
    congrats_bg = pygame.image.load("Graphic/bg.png")
    congrats_bg = pygame.transform.scale(congrats_bg, (width, hight))
    
    congrats_text = bigfont.render("Congratulations!", True, white)
    level_complete_text = font.render("You have completed Level 5!", True, white)
    continue_text = font1.render("Press M to go back to Menu", True, white)

    congrats_text_rect = congrats_text.get_rect(center=(width // 2, hight // 3))
    level_complete_text_rect = level_complete_text.get_rect(center=(width // 2, hight // 2.2))
    continue_text_rect = continue_text.get_rect(center=(width // 2, hight // 1.8))

    # วาดหน้าจอแสดงความยินดี
    ...
    # โหลดเพลงที่จะเล่น
    pygame.mixer.init()  # เริ่มการใช้งาน pygame mixer
    pygame.mixer.music.load("Graphic/endgame.mp3")  # โหลดไฟล์เพลง
    pygame.mixer.music.play(-1)  # เล่นเพลงแบบวนรอบ (-1 หมายถึงวนซ้ำไปเรื่อย ๆ)
    ...
    while waiting:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                return
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_m:  # กลับไปที่เมนู
                    pygame.mixer.music.stop()  # หยุดเพลงเมื่อกลับไปที่เมนู
                    waiting = False
                    return

# กำหนด level ในการผ่านแต่ละด่าน
level_thresholds = [5, 7, 8, 9, 4]  
# level ณ ปัจจุบัน
current_level = 1
...
def game():
    ...
    while True:
        ...
        if current_level == 4:
            ...
            if abs(dx) > abs(dy):  # ถ้าระยะห่างในแกน X มากกว่าในแกน Y
                ...
                if dy > 0:
                    ...
                    ai_direction = "UP"
            # เปลี่ยนแปลงตำแหน่งของงูในทิศทางต่างๆ ตามตัวแปร ai_direction ที่เก็บทิศทางที่งูกำลังเคลื่อนที่ไป
            if ai_direction == "UP":
                ai_snake_pos1[1] -= 5
            elif ai_direction == "DOWN":
                ai_snake_pos1[1] += 5
            ...
            # เพิ่มความยาวให้มากขึ้นทุกครั้ง
            ai_snake_body1.insert(0, list(ai_snake_pos1))  # เพิ่มส่วนใหม่
            for _ in range(3):  # เพิ่มจำนวนส่วนเพื่อให้ยาวขึ้น
                ai_snake_body1.insert(0, list(ai_snake_pos1))  # เพิ่มส่วนใหม่            
                ai_snake_body1.pop()  # ลบส่วนท้ายสุดออก
        ...
        if current_level > 5:  # ตรวจสอบว่าผู้เล่นผ่านเลเวล 5 แล้ว
            pygame.mixer.music.stop()  # หยุดเพลงพื้นหลัง
            congratulation_screen()  # แสดงหน้าจอแสดงความยินดี
            return
...
# เอาไว้วนลูป เพื่อให้เข้าฟังก์ชันที่เขียนไว้
def main():
    while True:
        menu_result = menu()
